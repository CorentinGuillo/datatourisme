...
@st.cache
def load_data():
    data1 = pd.read_csv('df_produit.zip', sep=' ', dtype={"code_departement":"str"}, error_bad_lines=False, low_memory = False)
...
@st.cache
def load_data():
    data2 = pd.read_csv('df_fete.zip', sep=' ', dtype={"code_departement":"str"}, error_bad_lines=False, low_memory = False)
...
@st.cache
def load_data():
    data4 = pd.read_csv('df_it.zip', sep=' ', dtype={"code_departement":"str"}, error_bad_lines=False, low_memory = False)
...
#		image = Image.open('DATAtourisme.png')
		...
		with col2:
			st.markdown("Dans le cadre de notre formation professionnelle de Data Analyst, *notre équipe* de 5 s'est alliée à *ADN Tourisme* pour proposer un *état des lieux* constamment à jour du *projet DATAtourisme* (qualité, quantité, points d'amélioration), ce qui n'existait pas jusqu'alors.", unsafe_allow_html=True)   
